 ...
 * deDupAndReverse, which is allowed to call numUnique and then go through the
...
public class CSC300Program3a {
	 ...
	 * not call any other functions. Here are some examples (using "=="  informally):
	 *
	 * <pre>
	 *     0 == numUnique(new int[] { })
	 *     1 == numUnique(new int[] { 11 })
	 *     1 == numUnique(new int[]{ 11, 11, 11, 11, 22, 33, 44, 44, 44, 44, 44, 55, 55, 66, 77, 88, 88 })
	...
	public static int[] deDupAndReverse (int[] list) {
		int numOfSlotsNeeded = numUnique(list);
		int totalIndex = list.length -1;
		int n = 0; //this is a counter to tell us which slot were on
		int [] Answer = new int[numOfSlotsNeeded];
		
		if (totalIndex==0 || numOfSlotsNeeded == 1)
			Answer[n] = list[0];
		
		if (totalIndex>1 && numOfSlotsNeeded > 1  ) {
	...
	public static void main (String[] args) {		
		...
		// for deDupAndReverse: array must be sorted
